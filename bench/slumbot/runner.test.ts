...
import { SlumbotClient, type FetchLike, type SlumbotResponse } from './client.js';
import { BIG_BLIND, replay, type Pos } from './protocol.js';
import { runSlumbot, summarizeSlumbot, type SlumbotHand } from './runner.js';
 ...
 */
function fakeServer(): { fetchImpl: FetchLike; requests: { path: string; body: Record<string, string> }[] } {
  ...
  const respond = (token: string): SlumbotResponse => {
    ...
    const res: SlumbotResponse = { action: s.action, client_pos: s.clientPos, hole_cards: ['As', 'Kd'], board, token };
    if (st.toAct === null) {
  ...
  const fetchImpl: FetchLike = async (url, init) => {
    ...
    if (path === 'new_hand') {
      ...
      const s = { action: '', clientPos: ((prev?.handNo ?? 0) % 2) as Pos, handNo: (prev?.handNo ?? 0) + 1 };
      sessions.set(token, s);
      botPlays(s);
    } else {
      const s = sessions.get(token ?? '');
      if (s === undefined) return { ok: true, status: 200, json: async () => ({ error_msg: 'bad token' }) };
      s.action += body['incr'] ?? '';
      const st = replay(s.action);
...
describe('runSlumbot', () => {
  it('plays whole hands with any agent and conserves the protocol', async () => {
    ...
    const { hands, partial } = await runSlumbot({ client, makeHero: () => createAgent('caller', 1), hands: 6, sessions: 2 });
    ...
    expect(requests.filter((r) => r.path === 'act').every((r) => typeof r.body['token'] === 'string')).toBe(true);
    expect(requests.filter((r) => r.path === 'new_hand' && r.body['token'] === undefined)).toHaveLength(2);
  });

  it('lets a folding hero lose exactly what it put in', async () => {
    const { fetchImpl } = fakeServer();
    const client = new SlumbotClient({ fetchImpl, retryDelayMs: 1 });
    const folder = { id: 'folder', decide: async (_v: unknown, legal: { canFold: boolean }) => (legal.canFold ? { type: 'fold' as const } : { type: 'check' as const }) };
  ...
  it('runs the heuristic hero without touching any backend, and stops early when aborted', async () => {
    ...
    const { hands, partial } = await runSlumbot({
      client, makeHero: () => new HeuristicAgent(), hands: 50, sessions: 1, signal: ctrl.signal,
      onHand: (done) => { if (done >= 3) ctrl.abort(); },
    });
  ...
  it('surfaces a server error instead of looping', async () => {
    const failing: FetchLike = async () => ({ ok: true, status: 200, json: async () => ({ error_msg: 'Illegal bet' }) });
    const client = new SlumbotClient({ fetchImpl: failing, retryDelayMs: 1 });
    await expect(runSlumbot({ client, makeHero: () => createAgent('caller', 1), hands: 2, sessions: 1 })).rejects.toThrow('Illegal bet');
  });
});

describe('summarizeSlumbot', () => {
  const hand = (netBB: number, clientPos: Pos, action: string): SlumbotHand => ({ session: 0, index: 0, clientPos, holeCards: [], board: [], botHoleCards: null, action, netBB, decisions: [], extra: {} });
  it('reports bb/100 with a CI over hands, the split by position, and the client VPIP/PFR', () => {
    const s = summarizeSlumbot([hand(1, 1, 'b250f'), hand(-0.5, 1, 'f'), hand(-1, 0, 'b200f'), hand(2, 0, 'b200c/kk/kk/kk')]);
    ...
    const based = [hand(1, 1, 'b250f'), hand(-1, 0, 'b200f')].map((h, i) => ({ ...h, extra: { baseline_winnings: i === 0 ? 50 : -150 } }));
