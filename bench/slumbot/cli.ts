...
import { createMockBackend, createTypeSafeBackend } from '../../src/jev/backend.js';
import { HeuristicAgent } from '../../src/jev/heuristic-agent.js';
import { getPersona } from '../../src/jev/personas.js';
import { SlumbotClient } from './client.js';
import { runSlumbot, summarizeSlumbot, type SlumbotSummary } from './runner.js';
...
function parse(argv: string[]): Options {
  const o: Options = { hero: 'jev', persona: 'tag', backend: 'typesafe', model: null, variance: null, hands: 200, sessions: 4, label: null };
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (flag) {
      case '--hero': {
        const v = take();
        if (!['jev', 'heuristic', 'rules', 'caller', 'random'].includes(v)) fail(`invalid --hero: ${v}`);
        o.hero = v as Hero;
        break;
      }
      case '--persona': o.persona = take(); break;
      case '--backend': {
        const v = take();
      ...
      case '--model': o.model = take(); break;
      case '--variance': {
        const v = Number(take());
        if (!Number.isFinite(v) || v < 0 || v > 1) fail('invalid --variance: expected a number from 0 to 1');
        o.variance = v;
        break;
      }
      case '--hands': o.hands = int(); break;
      case '--sessions': o.sessions = int(); break;
      case '--label': o.label = take(); break;
...
function table(hero: string, s: SlumbotSummary): string {
  ...
  const vbCell = vb === null ? 'n/a' : `${signed(vb.bb100)} ${vb.ci95 === null ? '' : `[${signed(vb.ci95[0])}, ${signed(vb.ci95[1])}]`}`.trim();
  const head = ['hero', 'hands', 'bb/100', '95% CI', 'vs baseline', 'sd/hand', 'as BB', 'as BTN', 'VPIP', 'PFR', 'fail-open', 'latency'];
  const row = [hero, String(s.hands), signed(s.bb100), ci, vbCell, `${s.sdPerHandBB.toFixed(1)} bb`, signed(s.asBigBlind.bb100), signed(s.asButton.bb100), `${Math.round(s.vpip * 100)}%`, `${Math.round(s.pfr * 100)}%`, String(s.failOpen), `${(s.meanLatencyMs / 1000).toFixed(1)}s`];
  return [`| ${head.join(' | ')} |`, `| ${head.map(() => '---').join(' | ')} |`, `| ${row.join(' | ')} |`].join('\n');
}

async function main(): Promise<void> {
  const opts = parse(process.argv.slice(2));
  if (opts.hero === 'jev' && opts.backend === 'typesafe' && (process.env['TYPESAFE_API_KEY'] ?? '') === '') fail('TYPESAFE_API_KEY is not set');

  const basePersona = getPersona(opts.persona);
  const persona = opts.variance === null ? basePersona : { ...basePersona, variance: opts.variance };
  const backend = opts.hero !== 'jev' ? null : opts.backend === 'typesafe' ? createTypeSafeBackend({ ...(opts.model !== null ? { model: opts.model } : {}) }) : createMockBackend();

  const makeHero = (session: number, onDecision: (r: DecisionRecord) => void): Agent => {
    if (opts.hero === 'jev') return new JevAgent({ persona, backend: backend!, seed: 1000 + session, onDecision });
  ...
  const { hands, partial } = await runSlumbot({
    ...
    onHand: (done, total) => {
      if (done % 25 === 0 || done === total) process.stderr.write(`[slumbot/${heroName}] ${done}/${total} hands\n`);
    },
  });
  ...
  const result = {
    ...
    partial,
    config: { hero: opts.hero, persona: opts.hero === 'jev' ? opts.persona : null, backend: opts.hero === 'jev' ? opts.backend : null, model: opts.model, variance: opts.variance, hands: opts.hands, sessions: opts.sessions, gitCommit: gitCommit() },
    summary,
    hands,
  };
