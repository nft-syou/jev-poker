...
import type { SlumbotClient, SlumbotResponse } from './client.js';
import { BIG_BLIND, encodeAction, heroView, type Pos } from './protocol.js';
...
export interface SlumbotRunOptions {
  ...
  makeHero: (session: number, onDecision: (r: DecisionRecord) => void) => Agent;
  hands: number;
...
const KNOWN = new Set(['old_action', 'action', 'client_pos', 'hole_cards', 'board', 'token', 'winnings', 'bot_hole_cards', 'error_msg']);

/** Play one hand to the end. Returns the final response and the token to use next. */
async function playOne(client: SlumbotClient, token: string | null, hero: Agent): Promise<{ last: SlumbotResponse; token: string | null }> {
 ...
 */
export async function runSlumbot(opts: SlumbotRunOptions): Promise<{ hands: SlumbotHand[]; partial: boolean }> {
  ...
  const session = async (id: number): Promise<void> => {
    ...
    for (;;) {
      ...
      try {
        ...
        for (const [k, v] of Object.entries(last)) if (!KNOWN.has(k) && typeof v === 'number') extra[k] = v;
        out.push({
          session: id,
          index,
...
export function summarizeSlumbot(hands: SlumbotHand[]): SlumbotSummary {
  ...
  const m = mean(xs);
  const sd = xs.length < 2 ? 0 : Math.sqrt(xs.reduce((a, x) => a + (x - m) ** 2, 0) / (xs.length - 1));
  ...
  const pre = hands.map((h) => {
    ...
    return { vpip: mine.some((t) => t === 'c' || t.startsWith('b')), pfr: mine.some((t) => t.startsWith('b')) };
  });
  ...
  if (withBase.length === hands.length && hands.length > 0) {
    ...
    const dm = mean(ds);
    const dsd = ds.length < 2 ? 0 : Math.sqrt(ds.reduce((a, x) => a + (x - dm) ** 2, 0) / (ds.length - 1));
    const dh = ds.length < 2 ? 0 : (1.96 * dsd) / Math.sqrt(ds.length);
    vsBaseline = { bb100: dm * 100, ci95: ds.length < 2 ? null : [(dm - dh) * 100, (dm + dh) * 100], sdPerHandBB: dsd };
  }
