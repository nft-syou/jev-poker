...
import { encodeAction, heroView, replay } from "./protocol";
...
describe("heroView", () => {
  it("describes the big blind facing a 2 bb open", () => {
    ...
    expect(view.history).toEqual([
      { street: "preflop", seat: 1, action: { type: "raise", amount: 200 } },
    ]);
  ...
  it("labels a first postflop bet as a bet and a raise over it as a raise", () => {
    ...
    expect(view.history.map((h) => [h.seat, h.action.type])).toEqual([
      [1, "raise"],
      [0, "call"],
      [0, "check"],
      [1, "bet"],
...
describe("encodeAction", () => {
  ...
  it("clamps and degrades instead of sending an illegal action", () => {
    ...
    expect(
      encodeAction(
        { type: "raise", amount: 900 },
        { ...facing, minRaiseTo: null, maxRaiseTo: null },
      ),
    ).toBe("c");
  });
});
