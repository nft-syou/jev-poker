...
describe("combos and ranges", () => {
  it("expands classes to 6 / 4 / 12 holdings", () => {
    ...
    expect(classOf(parseCards("9h Th"))).toBe("T9s");
    expect(() => combosOf("1x")).toThrow(/bad hand class/);
  });
...
describe("range inference", () => {
  ...
  const threeBet: HistoryEntry[] = [
    ...open,
    { street: "preflop", seat: 2, action: { type: "raise", amount: 900 } },
  ];
  it("reads width from preflop actions", () => {
    expect(preflopRangePercent(1, open)).toBe(20);
    expect(preflopRangePercent(2, threeBet)).toBe(6);
    expect(
      preflopRangePercent(3, [...open, { street: "preflop", seat: 3, action: { type: "call" } }]),
    ).toBe(25);
    expect(preflopRangePercent(4, [{ street: "preflop", seat: 4, action: { type: "call" } }])).toBe(
      50,
    );
    expect(preflopRangePercent(5, open)).toBe(100);
  });
  it("reads a shove (recorded as allin by historyEntry) like a raise", () => {
    expect(
      preflopRangePercent(1, [{ street: "preflop", seat: 1, action: { type: "allin" } }]),
    ).toBe(20);
    expect(
      preflopRangePercent(2, [...open, { street: "preflop", seat: 2, action: { type: "allin" } }]),
    ).toBe(6);
  });
...
describe("estimateEquityVsRanges", () => {
  it("is lower against a tight range than against random hands", () => {
    ...
    // The estimates converge to about 82 and 70; at 400 samples this stream gives 81 and 72.
    expect(vsTight).toBeLessThan(vsRandom - 5);
    expect(vsTight).toBeGreaterThan(60);
    expect(estimateEquityVsRanges(parseCards("7h 2d"), [], [topPercentRange(6)], 400)).toBeLessThan(
      25,
    );
  });
