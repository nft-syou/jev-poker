...
describe('handStrengthPct', () => {
  ...
  it('rates weak two pair below a set', () => {
    const weakTwoPair = handStrengthPct(parseCards('7h 2d'), parseCards('7c 2c Kd Ks 9h'))!; // 7s and 2s with KK on board = kings up
    const set = handStrengthPct(parseCards('Kh Kd'), parseCards('Kc 7c 2d'))!;
    expect(set).toBeGreaterThan(weakTwoPair);
    expect(set).toBeGreaterThan(95);
  });
  it('rates high card low on a coordinated board', () => {
    expect(handStrengthPct(parseCards('7h 2d'), parseCards('Ac Kc Qd'))!).toBeLessThan(20);
  });
});

describe('boardTexture', () => {
  it('detects paired, flush and straight boards', () => {
    expect(boardTexture(parseCards('Kc Kd 2h'))).toEqual({ paired: true, flushPossible: false, straightPossible: false });
    expect(boardTexture(parseCards('Kc 9c 2c'))).toEqual({ paired: false, flushPossible: true, straightPossible: false });
    expect(boardTexture(parseCards('9c 8d 5h'))).toEqual({ paired: false, flushPossible: false, straightPossible: true });
    expect(boardTexture(parseCards('Ac 3d 5h'))).toEqual({ paired: false, flushPossible: false, straightPossible: true });
    expect(boardTexture(parseCards('Ac 8d 2h'))).toEqual({ paired: false, flushPossible: false, straightPossible: false });
